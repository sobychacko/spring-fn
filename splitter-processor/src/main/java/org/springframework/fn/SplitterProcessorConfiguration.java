...
public class SplitterProcessorConfiguration {
	...
	private final SplitterProcessorProperties splitterProcessorProperties;

	public SplitterProcessorConfiguration(SplitterProcessorProperties splitterProcessorProperties) {
		this.splitterProcessorProperties = splitterProcessorProperties;
	}

	@Bean
	public Function<Message<?>, Flux<Message<?>>> splitterFunction(AbstractMessageSplitter messageSplitter) {
		messageSplitter.setApplySequence(this.splitterProcessorProperties.isApplySequence());
	...
	public AbstractMessageSplitter expressionSplitter() {
		return new ExpressionEvaluatingSplitter(
				new SpelExpressionParser()
						.parseExpression(this.splitterProcessorProperties.getExpression()));
	}
	...
	public AbstractMessageSplitter fileSplitter() {
		Boolean markers = this.splitterProcessorProperties.getFileMarkers();
		String charset = this.splitterProcessorProperties.getCharset();
		if (markers == null) {
			markers = false;
		}
		FileSplitter fileSplitter = new FileSplitter(true, markers, this.splitterProcessorProperties.getMarkersJson());
		if (charset != null) {
	...
	public AbstractMessageSplitter defaultSplitter() {
		...
		defaultMessageSplitter.setDelimiters(this.splitterProcessorProperties.getDelimiters());
